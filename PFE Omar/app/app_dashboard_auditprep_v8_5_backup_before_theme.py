# ============================================================
# AuditPrep IA - V8.5 finale entreprise
# Base : V8.4 sécurisée + interface portail métier Convergence
# Fichier à placer dans : C:\PFE Omar\app\app_dashboard_auditprep_v8_5_final.py
#
# Lancement PowerShell :
#   cd "C:\PFE Omar"
#   conda run -n auditprep python -m streamlit run "app\app_dashboard_auditprep_v8_5_final.py"
# ============================================================
...
# manquants, synthèse IA plus lisible et parcours guidé consolidé.
...
def augment_checklist_with_target_context(checklist_df, selected_run, targets_df, contexts_df=None):
    ...
    if not questions:
        return out, "Aucun contexte métier sauvegardé pour cette mission cible : le lot reste basé sur l’historique brut."
...
def render_brand_nav(active="Accueil", user_info=None):
    """Barre de marque horizontale inspirée des portails métier modernes."""
    if user_info:
        right_label = (
...
<div class="audit-topbar">
    ...
    <div class="audit-nav-items">
        <span>Accueil</span>
        <span>Préparation</span>
        <span>Résultats</span>
        <span class="audit-nav-active">{html.escape(active)}</span>
...
<div class="audit-user-chip">
    ...
    guided_mode = st.toggle(
        "Mode guidé",
        value=True,
        help="Affiche seulement le parcours utile. Les données, métriques et réglages complets restent accessibles dans les zones avancées.",
    )
    st.caption("Recommandé pour préparer et générer une check-list sans se perdre.")

    st.markdown("---")
    st.markdown("### Version")
    st.markdown(
    """
- Dashboard : **V8.5 interface portail métier**
- Moteur SQL : **V6**
- Sécurité : **authentification, rôles, session et secrets externalisés**
- Correction : **mode sombre / clair**
- Contrôle : **mission cible différente de l’historique**
- IA : **classification + régression sans fuite directe gravité/type**
- Données : **validation experte guidée + nettoyage du français**
- Décision : **fusion prudente règles/ML + supervision auditeur**
    """
)
...
<div class="audit-warning">
    <b>Principe de sécurité :</b> l’IA assiste la préparation mais ne remplace jamais la validation de l’auditeur.
</div>
        """,
        unsafe_allow_html=True,
    )
...
<div class="audit-card">
    <b>Parcours recommandé</b><br><br>
    <span class="audit-badge badge-low">1 · Vérifier l’IA</span>
    <span class="audit-badge badge-medium">2 · Préparer la mission</span>
    <span class="audit-badge badge-high">3 · Générer et contrôler</span><br><br>
    Les zones de création de données, les métriques détaillées et les anciens lots sont repliés.
</div>
        """,
        unsafe_allow_html=True,
    )
else:
    st.markdown("## Pourquoi cet outil ?")
    st.markdown(
        """
<div class="audit-grid-3">
    <div class="audit-mini-card"><b>PROBLÈME MÉTIER</b><h3>Préparation trop manuelle</h3><p>Les constats passés sont peu réutilisés.</p></div>
    <div class="audit-mini-card"><b>RÉPONSE AUDITPREP</b><h3>Capitaliser les signaux</h3><p>Classification, criticité et règles métier restent explicables.</p></div>
    <div class="audit-mini-card"><b>RÉSULTAT</b><h3>Une check-list ciblée</h3><p>Le livrable est priorisé, justifié et exportable.</p></div>
</div>
        """,
        unsafe_allow_html=True,
    )
    st.markdown("## Chaîne de valeur du prototype")
    st.markdown(
        """
...
if guided_mode:
    st.caption("Les outils d’alimentation de la base sont rangés ci-dessous et ne sont pas nécessaires pour un test normal.")
else:
    st.markdown("## Outils avancés : gérer les données")
    st.caption(
        "Cette zone sert à alimenter la base : créer une mission, compléter son contexte métier, ajouter/importer des constats. Elle ne lance pas la génération."
    )
    st.markdown(
        """
...
st.markdown(
    """
<div class="audit-note">
<b>Lecture projet :</b> V8.3 conserve le parcours guidé de la V8.2 et améliore la lisibilité des lots,
des décisions IA et des informations nécessaires à la validation humaine.
</div>
    """,
    unsafe_allow_html=True,
)
...
st.markdown('<hr class="audit-separator">', unsafe_allow_html=True)
st.markdown("## Étape 1 — Vérifier que le modèle est prêt")
st.caption(
    "Les constats historiques deviennent des exemples étiquetés. La classification prédit "
    "Faible / Moyenne / Haute ; la régression estime un score de criticité entre 0 et 100."
)
st.caption(
    "Sécurité V8.3 : la gravité et le type de constat sont exclus des entrées du modèle, "
    "car ils servent encore à construire certaines étiquettes proxy."
)
...
if guided_mode:
    with st.expander("Réglages du modèle (facultatif)", expanded=False):
        st.caption("Le modèle se réentraîne automatiquement lorsque les données changent.")
        ml_retrain = st.button(
            "Forcer le réentraînement",
            key="v82_train_models_guided",
            help="Les métriques sont calculées sur 30 % de données jamais vues pendant l'entraînement.",
        )
else:
    ml_retrain = st.button(
...
if expert_rows < 10:
    ...
    st.caption(
        f"Progression de la validation : {expert_rows}/{dataset_rows}. "
        "La bascule vers les seules données expertes dépend aussi de la présence d'au moins deux classes équilibrées."
    )
...
if ml_pack.get("status") == "dependency_missing":
    ...
    if guided_mode:
        st.success(
            f"Modèle prêt — {train_rows} exemples pour l’entraînement et {test_rows} pour le test. "
            "Tu peux passer à l’étape 2."
        )
        st.caption(
            f"Périmètre d'entraînement : {ml_pack.get('training_scope', 'non précisé')} "
            f"({model_dataset_rows} ligne(s)). Il reste {proxy_rows} étiquette(s) proxy."
        )
        guided_accuracy = float(class_metrics.get("accuracy", 0.0) or 0.0)
        guided_r2 = reg_metrics.get("r2", float("nan"))
        if dataset_rows < 30 or guided_accuracy < 0.70 or pd.isna(guided_r2) or guided_r2 < 0:
            st.info(
                "Lecture simple : le pipeline IA fonctionne, mais le volume ou les performances restent limités. "
                "Les règles métier sont donc conservées lorsque la prédiction n'est pas assez fiable."
            )
        with st.expander("Voir le résumé des performances IA", expanded=False):
            g1, g2, g3, g4 = st.columns(4)
            g1.metric("Accuracy", f"{100 * guided_accuracy:.1f} %")
            g2.metric("MAE", f"{reg_metrics.get('mae', 0):.2f}")
            g3.metric("RMSE", f"{reg_metrics.get('rmse', 0):.2f}")
            g4.metric("R²", "n/a" if pd.isna(guided_r2) else f"{guided_r2:.3f}")
            st.caption(
                "Accuracy mesure la classification des priorités. MAE et RMSE mesurent l'erreur du score sur 100. "
                "Un R² négatif indique que la régression n'est pas encore suffisamment fiable."
            )
    else:
        st.success(
    ...
    if not guided_mode:
        ...
        with ml_tab_class:
            ...
            with right:
                st.markdown("### Variables textuelles les plus influentes")
...
st.markdown("## Étape 2 — Choisir la mission et le mode")
st.caption(
    "Le moteur peut travailler avec un historique précis, proposer un historique pertinent, "
    "ou générer une première check-list sans historique."
)
...
context_container = (
    st.expander("Contexte métier — vérifier ou modifier seulement si nécessaire", expanded=False)
    if guided_mode else st.container()
)
with context_container:
    st.markdown("### Contexte métier de la mission")
    st.caption(
        "Les valeurs déjà enregistrées sont utilisées automatiquement. Modifie-les seulement si le contexte a changé."
    )
...
if generation_mode == "Avec historique manuel":
    ...
    if sources_df.empty:
        ...
        st.caption(
            "Le classement est explicable : il compare le contexte saisi, le client/site, le référentiel, les constats disponibles et les signaux historiques déjà calculés par le moteur SQL."
        )
        st.dataframe(
...
<b>Premier audit / sans historique :</b><br>
...
st.markdown("### Étape 3 — Lancer la génération")
st.caption("Vérifie les deux cartes ci-dessus, puis clique une seule fois sur le bouton.")
generate_btn = st.button(
    ...
    type="primary",
    use_container_width=guided_mode,
)
...
if generate_btn:
    try:
        if generation_mode == "Premier audit / sans historique":
            ...
            st.session_state.pop("v82_run_guided", None)
...
st.markdown('<hr class="audit-separator">', unsafe_allow_html=True)
st.markdown("## Étape 4 — Contrôler le résultat généré")
st.caption(
    "Le dernier lot généré est sélectionné automatiquement. Ouvre la liste seulement si tu veux consulter un ancien résultat."
)
...
if guided_mode:
    with st.expander("Consulter un autre lot (facultatif)", expanded=False):
        selected_run_label = st.selectbox("Lot à afficher", run_labels, index=preferred_index, key="v82_run_guided")
else:
    selected_run_label = st.selectbox("Lot à afficher", run_labels, index=preferred_index, key="v82_run_advanced")
...
<b>Contrôle supervisé V8.3 :</b> {ml_application_message}
    ...
    if guided_mode and not checklist_df.empty:
        guided_review_count = int(
            checklist_df.get("ml_review_required", pd.Series(dtype=bool))
            .fillna(False).astype(bool).sum()
        )
        if guided_review_count:
            st.warning(
                f"Contrôle humain conseillé pour {guided_review_count} question(s). "
                "Ouvre l’onglet « 2 · Check-list et validation » : la colonne « Explication simple » indique pourquoi."
            )
        else:
            st.success("Aucune divergence nécessitant une confirmation humaine n’a été détectée sur ce lot.")
...
st.markdown(
    f"""
<div class="audit-card">
    <h2>{safe_first(pd.DataFrame([selected_run]), 'target_mission_title', 'Mission cible')}</h2>
...
# ---------------------- TAB 1 : VUE EXÉCUTIVE ----------------------
with tab_exec:
    st.markdown("## Synthèse de la génération")
    st.caption("Vue globale du lot sélectionné et des priorités de préparation produites automatiquement.")
...
<div class="audit-grid-4">
    ...
    st.markdown(f"## {mode_labels['alerts_title']}")
...
st.markdown('<hr class="audit-separator">', unsafe_allow_html=True)
st.markdown("## 5. Exports du lot sélectionné")
